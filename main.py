...
async def report(department: str, location: str):
    ...
    big_hos_df = hos_df[hos_df['진료과정보'].str.contains("병원")]
    ...
    for k in range(len(big_hos_df)):
        hos_name = big_hos_df['의원명'].to_list()[k]
        hos_open_date = big_hos_df['개설일자'].to_list()[k]
        hos_area = str(int(big_hos_df['총면적(평)'].to_list()[k])) + "평"
        hos_prof = big_hos_df['전문의수'].to_list()[k]
        hos_type = big_hos_df['진료과정보'].to_list()[k]
        hos_class = big_hos_df['구분'].to_list()[k]
        hos_simcode = big_hos_df['심평원코드'].to_list()[k]
    ...
    intro = {
        ...
        "hospital_count": str(len(s_hn_holder)) + "개",
        "big_hospital_count": str(len(b_hn_holder)) + "개",
    ...
    for i in range(len(s_df)):
        ...
        this_hospital['open_year'] = str(s_df['개원년도'].to_list()[i]) + "년"
    ...
    if len(s_df) > 10:
        ...
        for i in range(len(s10_df)):
            ...
            this10_hospital['open_year'] = str(s10_df['개원년도'].to_list()[i]) + "년"
    ...
    for j in range(len(b_df)):
        ...
        this_big_hospital['open_year'] = str(b_df['개원년도'].to_list()[j]) + "년"
    ...
    intro['hospital_headers'].append(this_headers)
    ...
    market_df = pd.read_excel("시장분석용_정보/시장분석용_{}.xlsx".format(user_input_loc))
    hos_4div_df = pd.read_excel("분기별_병의원정보/분기별_병의원목록_{}.xlsx".format(user_input_loc))
    ...
    hos_divname_holder = ["2021년 2분기", "2021년 3분기", "2021년 4분기", "2022년 1분기"]
    ...
    for i in range(len(tt_holder)):
        ms_short = {}
        ms_short['class'] = str(tt_holder[i].year) + "년 " + str(tt_holder[i].month) + "월"
    ...
    for i in range(len(hc_long_holder)):
        hc_long = {}
        hc_long['class'] = str(hc_long_holder[i]) + "년"
        hc_long['value'] = hcl_count_holder[i]
        market_analysis['hospital_count_long_trend'].append(hc_long)

    for i in range(len(tt2_holder)):
        ppa_short = {}
        ppa_short['class'] = str(tt2_holder[i].year) + "년 " + str(tt2_holder[i].month) + "월"
    ...
    for i in range(len(that_yy_holder)):
        ppa_long = {}
        ppa_long['class'] = str(that_yy_holder[i]) + "년"
    ...
    recent_open_hos_df = recent_open_hos_df.drop(labels="index", axis=1)
    ...
    competitive_analysis = {
        ...
        "competition_rate": str(round(sum(analysis_psq_list), 2)) + "로",
        "address_dong": user_input_loc_p1,
        "department": user_input_type,
    ...
    if (str(round(sum(analysis_psq_list), 2))[-1] == "3") | (str(round(sum(analysis_psq_list), 2))[-1] == "6") | (
            str(round(sum(analysis_psq_list), 2))[-1] == "0"):
        competitive_analysis['competition_rate'] = str(round(sum(analysis_psq_list), 2)) + "으로"

    if new_average_ppm == "":
        competitive_analysis['new_hospital_average_profit'] = "정보 없음"

    if len(cc_holder) > 10:
        for i in range(10):
            this_hpd = {}
            this_hpd['class'] = "의원{}".format(i + 1)
            this_hpd['value'] = analysis_ppa_1000_list[i]
            competitive_analysis['hospital_profit_distribution_chart'].append(this_hpd)
    else:
    ...
    for j in range(len(recent_open_hos_df)):
        ...
        this_new_hospital['area'] = str(round(recent_open_hos_df['면적'][j])) + "평"
    ...
    for k in range(len(clh_df)):
        ...
        this_closed_hospital['area'] = str(round(clh_df['면적(평)'][k])) + "평"
    ...
    this_3y_percent = abs(
        round((round(my_avg_amt_long[-1]) - round(my_avg_amt_long[0])) / round(my_avg_amt_long[0]) * 100))
    ...
    user_analysis = {
        ...
        "address_dong": user_input_loc_p1,
        "department": user_input_type,
        "customer_male_ratio": str(m_percent) + "%",
        "customer_female_ratio": str(f_percent) + "%",
        "sex_compare": fm_percent_ident,
        "male_max_count_age": m_ident + "로",
        "male_max_count_ratio": str(mm_percent) + "%",
        "female_max_count_age": f_ident + "로",
        "female_max_count_ratio": str(ff_percent) + "%",
    ...
    if m_ident == "60대 이상":
        user_analysis['male_max_count_age'] = "60대 이상으로"
    if f_ident == "60대 이상":
        user_analysis['female_max_count_age'] = "60대 이상으로"

    this_sex_dist = {}
    this_sex_dist['class'] = "남성"
    this_sex_dist['value'] = m_percent
    user_analysis['sex_distribution_chart'].append(this_sex_dist)
    this_sex_dist = {}
    this_sex_dist['class'] = "여성"
    this_sex_dist['value'] = f_percent
    user_analysis['sex_distribution_chart'].append(this_sex_dist)

    age_dist_holder = ["20대", "30대", "40대", "50대", "60대 이상"]
    for j in range(5):
        this_male_age_dist = {}
        this_male_age_dist['class'] = age_dist_holder[j]
        this_male_age_dist['value'] = round(
            [m_sum_holder[i] / sum(m_sum_holder) for i in range(len(m_sum_holder))][j] * 100)
        user_analysis['male_age_distribution_chart'].append(this_male_age_dist)
    for k in range(5):
        this_female_age_dist = {}
        this_female_age_dist['class'] = age_dist_holder[k]
        this_female_age_dist['value'] = round(
            [f_sum_holder[i] / sum(f_sum_holder) for i in range(len(f_sum_holder))][k] * 100)
        user_analysis['female_age_distribution_chart'].append(this_female_age_dist)

    profit_dist_holder = ["정보 없음", "2천만원 미만", "2000만원~3000만원", "3000만원~4000만원", "4000만원~6000만원", "6000만원~8000만원",
                          "8000만원~1억원", "1억원 이상"]
    for j in range(8):
        this_profit_dist = {}
        this_profit_dist['class'] = profit_dist_holder[j]
        this_profit_dist['value'] = round(
            [ic_sum_holder[i] / sum(ic_sum_holder) for i in range(len(ic_sum_holder))][j] * 100)
        user_analysis['customer_profit_distribution_chart'].append(this_profit_dist)

    for i in range(len(tt_holder)):
        this_appc_short = {}
        this_appc_short['class'] = str(tt_holder[i].year) + "년 " + str(tt_holder[i].month) + "월"
        this_appc_short['value'] = round(my_avg_amt_short[i])
        user_analysis['average_profit_per_customer_short_chart'].append(this_appc_short)

    for i in range(len(my_avg_amt_long)):
        this_appc_long = {}
        this_appc_long['class'] = str(amt_long_year[i]) + "년"
