...
# 2022-05-04 제작 시작
# 오픈닥터 지역 분석 리포트 자동 생성 알고리즘


# 2022-06-10 추가 작업
# JSON화에 대한 피드백 적용이 목적입니다.
# 각 섹션을 results 라는 큰 틀에 담아 JSON 형식을 갖춰 결과값이 나오도록 합니다.
...
if (user_input_type == "가정의학과")|(user_input_type == "내과")|(user_input_type == "소아청소년과")|(user_input_type == "이비인후과"):
    type_cf = coughs
elif (user_input_type == "마취통증의학과")|(user_input_type == "재활의학과")|(user_input_type == "정형외과")|(user_input_type == "신경외과"):
    type_cf = bones
...
if len(s_hn_holder) == 0:
    ...
    print("         오픈닥터의 개원 입지 분석 시스템에서 {} 내 분석 대상 의원은 총 {}개 입니다.".format(user_input_loc_p1, str(len(s_hn_holder))))
...
market_size_12m = 1000
# 지난 12개월 중 시장규모 최대 (값, 시기)
print(rec12m_df.groupby('TA_YM').sum().sort_values(by='EST_HGA', ascending=False))
max_msize = round(rec12m_df.groupby('TA_YM').sum().sort_values(by='EST_HGA', ascending=False)['EST_HGA'].to_list()[0] / 10000)
max_year = int(str(rec12m_df.groupby('TA_YM').sum().sort_values(by='EST_HGA', ascending=False).reset_index()['TA_YM'].to_list()[0])[:4])
max_month = int(str(rec12m_df.groupby('TA_YM').sum().sort_values(by='EST_HGA', ascending=False).reset_index()['TA_YM'].to_list()[0])[-2:])
# 지난 12개월 중 시장규모 최소
min_msize = round(rec12m_df.groupby('TA_YM').sum().sort_values(by='EST_HGA', ascending=False)['EST_HGA'].to_list()[-1] / 10000)
min_year = int(str(rec12m_df.groupby('TA_YM').sum().sort_values(by='EST_HGA', ascending=False).reset_index()['TA_YM'].to_list()[-1])[:4])
min_month = int(str(rec12m_df.groupby('TA_YM').sum().sort_values(by='EST_HGA', ascending=False).reset_index()['TA_YM'].to_list()[-1])[-2:])

# 연도별 정보 따로 추려내기 (장기 추세용)
holding_df1 = market_df[market_df['year'] >= my_year - 2].groupby('year').sum().reset_index().sort_values(by='year')
holding_df2 = market_df[market_df['year'] >= my_year - 2].groupby('TA_YM').sum().reset_index().sort_values(by='TA_YM')
yy_holder = []
ss_holder = []
...
# 1평당 매출액 (단기 추세)
pp_short_cost = round(sum(rec12m_df.groupby('TA_YM').mean()['profit_per_area']) / len(rec12m_df.groupby('TA_YM').mean()['profit_per_area']) / 10000)
# 1평당 매출액 (장기 추세)
holding_df3 = market_df[market_df['year'] >= my_year - 2].groupby('year').mean().reset_index().sort_values(by='year')
ppyy_holder = []
ppss_holder = []
...
# 연도별 의원수 확인
hos_2020_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2020_4") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2021_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2021_4") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
...
print("         지난 12개월 중 최대값은 {}만 원 ({}년 {}월) 이며, 최소값은 {}만 원 ({}년 {}월) 입니다.".format(max_msize, max_year, max_month, min_msize, min_year, min_month))
if early_size >= 10000:
    early_size = str(early_size // 10000) + "억 " + str(early_size % 10000)
if late_size >= 10000:
    late_size = str(late_size // 10000) + "억 " + str(late_size % 10000)
print("         최근 3년간 시장규모 추세는 {}년 {}만 원에서 {}년 {}만 원으로 {}% {}하였습니다.".format(early_year, early_size, late_year, late_size, abs(three_percent), three_ident))
print()
print("         {}의 현재 {} 의원 개수는 {}개 입니다.".format(user_input_loc_p1, user_input_type, hos_now_count))
if count_tendency == "유지":
    print("         최근 3년간 의원 개수 추세는 2020년 {}개에서 2022년 {}개로 {}개 {}되었습니다..".format(hos_2020_count, hos_now_count, abs(hos_now_count - hos_2020_count), count_tendency))
else:
    print("         최근 3년간 의원 개수 추세는 2020년 {}개에서 2022년 {}개로 {}개 {}하였습니다.".format(hos_2020_count, hos_now_count, abs(hos_now_count - hos_2020_count), count_tendency))
print()
print("         평수가 큰 의원일수록 매출이 높은 경향이 있기 때문에, 평수의 효과를 제거한 1평당 매출액을 확인하는 것이 중요합니다.")
print("         {} 의원 1평당 매출액은 월 {}만 원으로, 50평 의원 기준으로 환산하면 {}만 원입니다.".format(user_input_loc_p1, pp_short_cost, pp_short_cost*50))
print("         최근 3년 추세는 {}년 {}만 원에서 {}년 {}만 원으로 {}% {}하였습니다.".format(p_early_year, p_early_size, p_late_year, p_late_size, abs(three_percent2), three_ident2))
print()
...
hos_2020_1st_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2020_1") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2020_2nd_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2020_2") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2020_3rd_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2020_3") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2020_4th_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2020_4") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2021_1st_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2021_1") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2021_2nd_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2021_2") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2021_3rd_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2021_3") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
hos_2021_4th_count = len(hos_4div_df[(hos_4div_df['행정동코드'] == user_input_loc) & (hos_4div_df['분기'] == "2021_4") & (hos_4div_df['진료과분류'].str.contains(user_input_type))])
...
for i, v in enumerate(["2020", "2021", "2022"]):
    plt.text(v, [hos_2020_4th_count, hos_2021_4th_count, hos_now_count][i], [hos_2020_4th_count, hos_2021_4th_count, hos_now_count][i],
             fontsize=11,
             color="black",
             horizontalalignment='center',
             verticalalignment='bottom')
...
for n in range(len(tt2_holder)):
    market_analysis['12m_trend_chart']['profit_per_area_size_{}m'.format(n+1)] = int(round(t2['profit_per_area'] / 10000)[n])
...
# 각종 필요한 정보 산출해내기
# 전체 의원 평균 매출액
analysis_profit_list = vs_analysis_df.groupby('sim_cd').sum().sort_values(by="EST_HGA", ascending=False)['EST_HGA'].to_list()
# 해당 의원 코드들 리스트
analysis_simcd_list = vs_analysis_df.groupby('sim_cd').sum().sort_values(by="EST_HGA", ascending=False).reset_index()['sim_cd'].to_list()
# 해당 의원들 개수 리스트
analysis_count_list = []
...
if len(vs_newhos_name_holder) == 0:
    ...
    print("         최근 24개월 내 개원한 신규 의원은 {}개이며, 이들의 평균 매출액은 {}만 원으로 {} 전체 평균보다 {}.".format(len(vs_newhos_opendate_holder), new_average_ppm, user_input_loc_p1, vs_quali))
...
if len(analysis_ppa_1000_list) > 10:
    for i in range(len(analysis_ppa_1000_list[:10])):
        if analysis_ppa_1000_list[:10][i] > 10000:
            vvs1.append(str(int(analysis_ppa_1000_list[:10][i] // 10000))+ "억 " + str(round(analysis_ppa_1000_list[:10][i] % 10000, 1)) + "만 원")
        else:
    ...
    for i in range(len(analysis_ppa_1000_list)):
        if analysis_ppa_1000_list[i] > 10000:
            vvs1.append(str(int(analysis_ppa_1000_list[i] // 10000))+"억 " + str(round(analysis_ppa_1000_list[i] % 10000, 1)) + "만 원")
        else:
...
m_sum_holder = []
ic_sum_holder = []
fm_sum_holder.extend([sum(amt_f20_holder), sum(amt_f30_holder), sum(amt_f40_holder), sum(amt_f50_holder), sum(amt_f60_holder), sum(amt_m20_holder), sum(amt_m30_holder), sum(amt_m40_holder), sum(amt_m50_holder), sum(amt_m60_holder)])
f_sum_holder.extend([sum(amt_f20_holder), sum(amt_f30_holder), sum(amt_f40_holder), sum(amt_f50_holder), sum(amt_f60_holder)])
m_sum_holder.extend([sum(amt_m20_holder), sum(amt_m30_holder), sum(amt_m40_holder), sum(amt_m50_holder), sum(amt_m60_holder)])
ic_sum_holder.extend([sum(amt_ic0000_holder), sum(amt_ic0002_holder), sum(amt_ic0203_holder), sum(amt_ic0304_holder), sum(amt_ic0406_holder), sum(amt_ic0608_holder), sum(amt_ic0810_holder), sum(amt_ic1099_holder)])
...
my_avg_amt_short = ((market_df[market_df['TA_YM'] >= my_date].groupby("TA_YM").sum())['EST_HGA'] / (market_df[market_df['TA_YM'] >= my_date].groupby("TA_YM").sum())['EST_CNT']).to_list()
...
# B. Text
print("  B. Text")
print("      i. {}에서 {}에 가장 많은 금액을 지불한 고객군은 {} 입니다. 성별로 보면 남성 {}%, 여성 {}%로 {}이 더 많습니다.".format(user_input_loc_p1, user_input_type, fm_ident, m_percent, f_percent, fm_percent_ident))
print("         남성 중 가장 큰 비중을 차지하는 연령대는 {}로, 전체 남성 중 {}%에 해당합니다.".format(m_ident, mm_percent))
print("         여성의 경우는 {}로, 전체 여성 중 {}%를 차지합니다. 소득수준별로 보면 {} 구간이 전체의 {}%로 가장 많습니다.".format(f_ident, ff_percent, ic_ident, ic_percent))
...
print("         지난 12개월 중 최대값은 {}원 ({}년 {}월) 이며, 최소값은 {}원 ({}년 {}월) 입니다.".format(this_max_amt, amt_short_year[my_avg_amt_short.index(max(my_avg_amt_short))]//100, amt_short_year[my_avg_amt_short.index(max(my_avg_amt_short))]%100, this_min_amt, amt_short_year[my_avg_amt_short.index(min(my_avg_amt_short))]//100, amt_short_year[my_avg_amt_short.index(min(my_avg_amt_short))]%100))
...
print("         최근 3년 추세는 {} 원에서 {} 원으로 {}% {}하였습니다.".format(this_3y_first_amt, this_3y_last_amt, this_3y_percent, maal_ident))
print()
...
print("         1. 성별 분포")
plt.pie([m_percent, f_percent], labels=['Male', 'Female'], startangle=276, autopct="%.0f%%", wedgeprops={'width': 0.7, 'edgecolor': 'w', 'linewidth': 5})
plt.show()
print()
print("         2. 남성 연령 분포")
plt.pie([m_sum_holder[i] / sum(m_sum_holder) for i in range(len(m_sum_holder))], labels=['20', '30', '40', '50', '60up'], startangle=60, counterclock=False, autopct="%.0f%%", wedgeprops={'width': 0.7, 'edgecolor': 'w', 'linewidth': 5})
plt.show()
print()
print("         3. 여성 연령 분포")
plt.pie([f_sum_holder[i] / sum(f_sum_holder) for i in range(len(f_sum_holder))], labels=['20', '30', '40', '50', '60up'], startangle=60, counterclock=False, autopct="%.0f%%", wedgeprops={'width': 0.7, 'edgecolor': 'w', 'linewidth': 5})
plt.show()
print()
print("     ii. 소득수준별 매출 분포 [DONUT]")
plt.pie([ic_sum_holder[i] / sum(ic_sum_holder) for i in range(len(ic_sum_holder))], labels=['0000', '2000down', '2~3000', '3~4000', '4~6000', '6~8000', '8000~10000', '10000up'], startangle=90, counterclock=False, autopct="%.0f%%", wedgeprops={'width': 0.7, 'edgecolor': 'w', 'linewidth': 5})
plt.show()
print()
